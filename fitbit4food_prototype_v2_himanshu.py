...
def recommendation_engine_gui():
    ...
    diet_health_option = ["Vegetarian", "Vegan", "Halal", "Keto", "Paleo", "Nut Free", "Gluten Free", "Sugar Free",
                          "Non GMO", "Low Fat"]

    environment_option = ["Organic", "Free Range", "Carbon Neutral", "Feed Lot", "Part Vegetable Protein", "Water Safe",
                          "Low Irrigation", "Seaweed Fed", "Grain Finished", "Solar Power Produced", "Grass Fed"]

    packaging_option = ["Recycled", "Plastic", "FSC Certified", "PCT Plastic", "Recyclable Plastic",
                        "Recyclable Plastic Level 2"]

    socialization_option = ["Farmer Video", "Fair Trade", "Non Toxic Working Conditions",
                            "Donates Meals To Children In Need (Meat The Need)", "Living Wage For Workers"]

    my_preference_preset = 'None'

    # preset user profile
    preset = ["None", "Healthy Helena", "Sustainable Sally", "Dietary Dave", "Price Conscious Peter",
              "Only Organic Olivia"]
    ...
    if choice == "Home":
        ...
        tabs = ["Countdown", "Amazon"]
        if "tab" in query_params:
            active_tab = query_params["tab"][0]
        else:
            active_tab = "Countdown"

        if active_tab not in tabs:
            st.experimental_set_query_params(tab="Countdown")
            active_tab = "Countdown"
        ...
        if active_tab == "Countdown":
            ...
            if submit or product_name:
                ...
                try:
                    ...
                    myrows = zip(sub_recommendations['URL'], sub_recommendations['ProductTitle'],
                                 sub_recommendations['ProductImage'], sub_recommendations['ProductPrice'],
                                 sub_recommendations['ProductVolume'], sub_recommendations['Category'],
                                 sub_recommendations['ProductDetail'], sub_recommendations['Ingredients'],
                                 sub_recommendations['Nutritional_information'], sub_recommendations['Allergenwarnings'],
                                 sub_recommendations['Claims'], sub_recommendations['Endorsements'],
                                 sub_recommendations['Productorigin'], sub_recommendations['predicted_tags'])
                    ...
                    col1, col2, col3, col4, col5, col6, col7, col8, col9, col10, col11, col12, col13, col14) in enumerate(
                        ...
                        # convert predicted_tag string to list
                        predicted_tags_html = ""
                        if len(col14) > 4:
                            import ast
                            col14_array = ast.literal_eval(col14)
                            predicted_tags_html = '''<div class="mt-1 mb-1 spec-1">'''
                            for i in col14_array:
                                predicted_tags_html += '''<span class="dot"></span> <span>{value}</span>'''.format(value=i)
                            predicted_tags_html += "<br></div>"
            ...
            if submit or product_name:
                ...
                # Manage sorting option
                if sort_by == 'Price Low to High':
                    recommendations['Product_Price'].fillna(99999.0, inplace=True)
                    recommendations['Product_Price'] = recommendations['Product_Price'].astype(str)
                    # recommendations['Product_Price'].str.extract('(\d*\.?\d*)', expand=False).astype(float)
                    recommendations = recommendations.sort_values(by=['Product_Price'], ascending=True)

                elif sort_by == 'Price High to Low':
                    recommendations['Product_Price'].fillna(0.0, inplace=True)
                    recommendations['Product_Price'] = recommendations['Product_Price'].astype(str)
                    # recommendations['Product_Price'].str.extract('(\d*\.?\d*)', expand=False).astype(float)
                    recommendations = recommendations.sort_values(by=['Product_Price'], ascending=False)

                elif user_preset == 'Price Conscious Peter' or sort_by == 'Unit Price Low to High':
                    recommendations['Availability'].fillna("", inplace=True)
                    recommendations['Availability'] = recommendations['Availability'].astype(str)
                    recommendations = recommendations.sort_values(by=['Availability'], ascending=False)
                else:
                    pass
                ...
                try:
                    ...
                    myrows = zip(sub_recommendations['URL'], sub_recommendations['Product_Title'],
                                 sub_recommendations['Product_Image'], sub_recommendations['Product_Price'],
                                 sub_recommendations['Availability'], sub_recommendations['Important_Information'],
                                 sub_recommendations['Product_Description'], sub_recommendations['Product_Detail'],
                                 sub_recommendations['Special_Ingredients'], sub_recommendations['Allergen_Info'],
                                 sub_recommendations['Product_Info'], sub_recommendations['Rating'], sub_recommendations['Product_Reviews'])
                    ...
                    col1, col2, col3, col4, col5, col6, col7, col8, col9, col10, col11, col12, col13) in enumerate(
                        ...
                        PRODUCT_CARD += '''
                                ...
                                <div class="modal" id="card_{idx}">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <p>
                                            <button class="btn btn-primary" type="button" data-toggle="collapse" data-target="#special_ingredients" style = "background-color: #2e6f22; border-color: #2e6f22" aria-expanded="false" aria-controls="collapse">Special Ingredients</button>
                                            </p>
    
                                            <div class="collapse" id="special_ingredients">
                                                <div class="card card-body">
                                                    {special_ingredients}
                                                </div>
                                                <br>
                                ...
                                <script type = "text/javascript">
                                <!--
                                    var product_data_{idx} = {{ URL:"{product_link}", Product_Title:"{title}",tag:"",Product_Price:"{price}",Availability:"{availability}",Special_Ingredients:"{special_ingredients}",Product_Detail:"{product_detail}",Product_Description:"{product_description}",Product_Info:"{product_information}",Allergen_Info:"{allergen_information}",Important_Info:"{important_information}",Product_Image:"{img_link}",Rating:"rating",Product_Reviews:"{product_reviews}"}}
                            ...
                            '''.format(idx=idx, product_link=col1, title=col2, img_link=col3, price=col4, availability=col5,
                                       special_ingredients=col9, product_detail=col7, product_description=col8,
                                       product_information=col11, allergen_information=col10, important_information=col6, rating=col12, product_reviews=col13)
